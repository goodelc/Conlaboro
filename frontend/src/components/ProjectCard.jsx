...
export default memo(function ProjectCard({ p }) {
  ...
  return (
    <div className="project-card reveal visible" onClick={() => navigate(`/detail/${project.id}`)}>
      <span className={`card-tag ${TAG_CLASS[project.status]}`}>{STATUS_MAP[project.status]}</span>
      <h3>{project.title}</h3>
      ...
      <div className="card-footer">
        <div className="members">
          {allMembers.length > 0 ? (
            ...
            <span className="no-members">等待加入</span>
          ) : null}
        </div>
        <span className="join-btn">{project.status === 'done' ? '查看成果 →' : '查看详情 →'}</span>
      </div>
    </div>
  )
})
